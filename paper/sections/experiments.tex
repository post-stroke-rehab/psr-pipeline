...
The project has also evolved over time. Historical direct baselines remain under \texttt{metrics/}, while newer CNN outputs live under \texttt{models/CNN/evaluations/}. We keep both because they jointly show how the study matured from an initial cross-family comparison into a more deployment-oriented CNN branch. When a newer optimized artifact supersedes an older baseline, the paper says so explicitly rather than averaging across incompatible runs.
...
Because the benchmark mixes exact-match, per-finger, and ranking-sensitive metrics, the evaluation section is organized deliberately. We first compare the three direct model families under the shared preprocessing path. We then examine whether the newer CNN branch materially changes the frontier by comparing optimized student variants against the internal ResNet teacher family merged through PR~56. Finally, we treat transfer learning and distillation as improvement pathways rather than as baseline-defining results, because their artifact completeness differs.
